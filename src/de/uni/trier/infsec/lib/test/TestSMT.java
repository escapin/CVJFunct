...
public class TestSMT extends TestCase {
	...
	@Test
	public void testRealPKIRemote() throws PKIError, NetworkError, Exception, SMTError {
		Process pr = null;
		try {
			...
			Channel c1 = p1.channelTo(TEST_ID2, "localhost", NetworkServer.LISTEN_PORT);
			c1.send(TEST_DATA);
			Thread.sleep(1000);
			...
			error = false;
			try {
				p1.channelTo(TEST_ID2 + 1, "localhost", NetworkServer.LISTEN_PORT);
			} catch (PKIError e) {
				error = true;
			}
